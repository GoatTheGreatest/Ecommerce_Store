          ...
          <div className="lg:col-span-3">
            <div className="card">
              <div className="flex border-b border-gray-200">
                {['Description', 'Reviews', 'Shipping', 'About seller'].map((tab) => (
                  <button 
                    ...
                    className={`px-6 py-4 font-medium text-sm ${activeTab === tab ? 'text-primary border-b-2 border-primary' : 'text-[#8B96A5] hover:text-primary transition-colors'}`}
                  >
                    {tab}
                  </button>
                ))}
              ...
              <div className="p-6 text-[#505050] space-y-4">
                ...
                {activeTab === 'Reviews' && (
                  <div>
                    <h3 className="font-bold text-lg mb-2 text-[#1C1C1C]">Customer Reviews</h3>
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-orange-400 font-bold text-2xl">{product.rating}</span>
                      <div className="flex text-orange-400">
                        {[1, 2, 3, 4, 5].map(s => <span key={s}>{s <= Math.floor(product.rating) ? "★" : "☆"}</span>)}
                      </div>
                      <span className="text-sm text-[#8B96A5]">({product.reviews} reviews)</span>
                    </div>
                    <p className="text-sm text-[#8B96A5] italic">Customer reviews will appear here.</p>
                  </div>
                )}
                {activeTab === 'Shipping' && (
                  <div>
                    <h3 className="font-bold text-lg mb-2 text-[#1C1C1C]">Shipping Information</h3>
                    <ul className="list-disc pl-5 space-y-2 text-sm">
                      <li>Free shipping on orders over $50.</li>
                      <li>Ships from: <span className="font-medium text-[#1C1C1C]">{product.supplierRegion || "Global warehouse"}</span></li>
                ...
                {activeTab === 'About seller' && (
                  <div>
                    <h3 className="font-bold text-lg mb-2 text-[#1C1C1C]">About {product.sellerName || product.supplierName || "the Seller"}</h3>
                    <p className="text-sm mb-3">This product is sold and fulfilled by a verified independent seller on our platform.</p>
                    <div className="space-y-1">
                      <p className="text-sm"><span className="text-[#8B96A5] inline-block w-20">Location:</span> <span className="font-medium text-[#1C1C1C]">{product.supplierRegion || "Global"}</span></p>
                      <p className="text-sm"><span className="text-[#8B96A5] inline-block w-20">Website:</span> <a href={`https://${product.supplierUrl || "store.com"}`} target="_blank" rel="noreferrer" className="text-primary font-medium hover:underline">{product.supplierUrl || "Visit Store"}</a></p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                {relatedProducts.map(p => (
                  <Link href={`/product/${p._id}`} key={p._id} className="flex gap-3 group">
                    <div className="relative w-16 h-16 border border-gray-200 rounded overflow-hidden shrink-0 bg-white">
